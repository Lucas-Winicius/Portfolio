...
function ProjectContainer({ data }: { data: ProjectType }) {
  function copyLink() {
    ...
    navigator.clipboard.writeText(`${local}/project/v/${data.id}`);
  }
    ...
    <section className="flex flex-row items-center space-x-3 w-3/4 h-max">
      ...
      <div className="flex flex-col justify-between w-full space-y-3 h-max">
        <div className="space-y-2">
          <button
            className="float-right text-zinc-700 hover:text-zinc-100 hover:scale-110 transition-all"
            onClick={copyLink}
          >
            <LinkIcon size={15} />
          </button>
          ...
          <div className="flex flex-row space-x-4">
            {data.technologies.map((tecName, id) => (
              <div
                key={id}
                className="bg-red-600 px-2 py-1 rounded-sm text-xs font-semibold"
              >
                {tecName}
              </div>
            ))}
        ...
        <div className="flex flex-row justify-between items-end space-x-3">
          <div className="flex space-x-3">
            <Link
              href={data.github}
              className="flex items-center space-x-2 bg-pink-600 py-2 px-4 rounded hover:bg-pink-700 transition-colors arrow-animation"
            >
          ...
          <div>
            <p className="opacity-40 text-sm">
              Postado {formatDateToNow(data.createdAt)}
            </p>
          </div>
        </div>
      </div>
